    ...
    <div className="flex flex-col border-t border-white/10 mt-12">
      {projects.map((project, index) => (
        <motion.div
          ...
          className="group relative flex flex-col lg:flex-row lg:items-start justify-between gap-8 py-12 md:py-16 border-b border-white/10 transition-colors hover:bg-[#0A0A0A] px-4 md:px-8"
        >
          {/* Left: Huge Title & Date */}
          <div className="flex flex-col gap-4 lg:w-1/3">
            <span className="font-Silkscreen text-xs text-white/30 uppercase tracking-widest group-hover:text-[#0D8FB7] transition-colors duration-500">
              {project.date}
            </span>
            <h3 className="text-4xl md:text-5xl font-medium tracking-tight text-white/90 group-hover:text-white transition-colors duration-500">
              {project.title}
            </h3>
